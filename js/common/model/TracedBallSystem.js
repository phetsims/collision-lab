...
/**
 * TracedBallSystem is a BallSystem sub-type that contains trailing 'Paths' that trace the position of the CenterOfMass
 * and the Balls in the BallSystem. See BallSystem for context on the super class.
 *
 ...
 *
 * TracedBallSystems are created at the start of the sim and are never disposed, so no dispose method is necessary
...
class TracedBallSystem extends BallSystem {
  ...
  constructor( initialBallStates, playArea, elapsedTimeProperty, options ) {
    ...
    // and Paths are visible. DerivedProperty is never disposed since TracedBallSystems are never disposed.
  ...
  reset() {
    ...
    // Now let the super-reset its Properties.
    super.reset();
  }

  /**
   * Restarts the TracedBallSystem.
   * @public
   * @override
   *
...
collisionLab.register( 'TracedBallSystem', TracedBallSystem );
export default TracedBallSystem;